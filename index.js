...
console.log(
  degerlendirmeler.find((degerlendirmeler) => degerlendirmeler.isim === "Ahmet")
);
...
const userReyna = degerlendirmeler.find(
  (degerlendirmeler) => degerlendirmeler.isim === "Reyna"
);
userReyna.geribildirim =
...
function AnahtardanDegerlendirmeAl(arr, index) {
  ...
  if (obj !== null) {
    return `${obj.isim} isimli kişi ${obj.puan} puan verdi ve şunları yazdı: ${obj.geribildirim}`;
  } else {
    return 0;
  }
}
...
function PuanaGoreDegerlendirmeAl(arr, point) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].puan >= point && arr[i].puan < point + 1) {
...
function UzunDegerlendirmeleriAl(arr) {
	...
	let numberOfWords = [];
	for (let i = 0; i < arr.length; i++) { 
	numberOfWords = arr[i].geribildirim.split(' ')
		if( numberOfWords.length>15){
			longFeedbacks.push(arr[i])
		}
	}
	console.log('uzun geribildirimler')
	console.log(longFeedbacks)

	return longFeedbacks;
}
...
function arabaYapici(name) {
  this.isim = name;
  this.kmSayaci = 0;
  this.surus = function (km){
	this.kmSayaci += km;
  }
}

const araba1= new arabaYapici('araba1')
